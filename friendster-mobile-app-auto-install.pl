...
use constant SQL_BOTS_INSTALLER => "select * from bots where status = '' and type = 'installer';";
use constant SQL_APPS => "select * from apps;";
use constant SQL_BOTS_CRAWLER => "select * from bots where status = '' and type = 'crawler';";
		...
		if(@api_key)
		{
			api_key: foreach $api_key(@api_key)
			{
				...
				if(@ref_sql_apps)
				{
					...
					if(!$crawler->{crawler}->find(FILE_WIDGET_INFO, $url_xml_activate))
					{
						...
						#while($error)
						{
							$crawler->browse(URL_WIDGET_INFO, FILE_WIDGET_INFO, $wget_activate . " --read-timeout 1"); # --no-iri
							$error = $crawler->{crawler}->find(FILE_WIDGET_INFO, 'errorbox', 1);
						}
					}
				}
				else
